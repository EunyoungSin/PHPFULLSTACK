...
// promise1
// .then(data => {console.log(data);}) // 성공적으로 수행 했을 때 실행되는 코드
// .catch(error => {console.log(error);}) // 실패 했을 때 실행되는 코드
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                if(id !== '' && pw !== '') {
                    ...
                    reject(new Error('잘못 입력 하셨습니다.')); // 객체 생성
                }
            }, 500);
        });
    }
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                if(id === 'php506' && pw === '506') {
                    ...
                    reject(new Error('없는 유저입니다.')); // 객체 생성
                }
            }, 500);
        });
    }
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                if(id === 'php506') {
                    ...
                    reject(new Error('권한이 없습니다.')); // 객체 생성
                }
            }, 500);
        });
    }
