...
$stmt->bind_param("iii", $i1, $i2, $i3); // Prepare 셋팅
$stmt->execute();// DB에 쿼리 질의 실행

//-------------------- 질의 결과를 우리가 지정한 변수에 담아 사용하는 방법 -------------------
$stmt->bind_result( $col1, $col2 ); // 질의 결과를 각 argument에 저장하기 위한 셋팅
// $stmt->fetch(); // bind_result에서 셋팅한 argument에 값을 저장(한번 실행 될때마다 한 레코드씩 저장)
